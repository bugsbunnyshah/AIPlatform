...
{	
	...
	static public HashMap<Integer, RepositorySearchResult> search(String keywordString, ArrayList<Integer> tagIDs, SchemaStoreClient client) throws RemoteException
	{
		...
		ArrayList<ArrayList<org.mitre.schemastore.search.Match>> keywordMatches = new ArrayList<ArrayList<org.mitre.schemastore.search.Match>>();
		for(String keyword : keywords)
		{
			// Generate the array of matches for this keyword
			ArrayList<org.mitre.schemastore.search.Match> matches = new ArrayList<org.mitre.schemastore.search.Match>();
			...
			for(Schema schema : client.getSchemas())
				if(taggedSchemas.size()==0 || taggedSchemas.contains(schema.getId()))
					if(matches(schema.getName(),keyword) || matches(schema.getDescription(),keyword))
						matches.add(new org.mitre.schemastore.search.Match(schema));
		...
		for(Integer schemaID : matchedSchemas)
		{
			...
			ArrayList<org.mitre.schemastore.search.Match> schemaMatches = new ArrayList<org.mitre.schemastore.search.Match>();
			for(ArrayList<org.mitre.schemastore.search.Match> matches : keywordMatches)
