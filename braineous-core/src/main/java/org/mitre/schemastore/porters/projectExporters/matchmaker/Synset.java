...
//		nodes.ensureCapacity(nodes.size() + two.nodes.size());
	...
	float completeLinkage(Synset two) {
		...
		for (i = 0; i < this.terms.size(); i++) {
			org.mitre.schemastore.porters.projectExporters.matchmaker.Node t_node = this.terms.get(i);
			for (j = 0; j < two.terms.size(); j++) {
				org.mitre.schemastore.porters.projectExporters.matchmaker.Node s_node = two.terms.get(j);
