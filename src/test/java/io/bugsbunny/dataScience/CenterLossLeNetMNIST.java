...
public class CenterLossLeNetMNIST {
    private static final Logger logger = LoggerFactory.getLogger(CenterLossLeNetMNIST.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        recordReader.initialize(new FileSplit(new ClassPathResource("dataLake/digits.csv").getFile()));
        //CSVRecordReader recordReader2 = new CSVRecordReader();
        //recordReader2.initialize(new FileSplit(new File("iris_copy.txt")));
        //recordReader.reset();
        //while(recordReader.hasNext())
        //{
        //    loggerger.info(recordReader.next().toString());
        //}

        //DataSetIterator mnistTrain = new MnistDataSetIterator(batchSize, true, 12345);
        //DataSetIterator mnistTest = new MnistDataSetIterator(10000, false, 12345);
        DataSetIterator iter = new RecordReaderDataSetIterator.Builder(recordReader, 32)
        ...
        for (int i = 0; i < nEpochs; i++) {
            ...
            logger.info(testData.getFeatures().toString());
            logger.info(testData.getLabels().toString());
        ...
        //model.save(new File("model.in"));

        //InputStream is = new FileInputStream("model.in");
        //String modelString = IOUtils.toString(is, StandardCharsets.UTF_8);
        //modelString = Base64.getEncoder().encodeToString(modelString.getBytes(StandardCharsets.UTF_8));
