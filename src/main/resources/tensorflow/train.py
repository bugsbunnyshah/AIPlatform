...
if __name__ == "__main__":
    #trainingArray = np.asarray(csv,dtype=None)
    #trainingArray = np.genfromtxt('dataset.csv', delimiter=',', dtype=np.float64)
    #X = trainingArray
    #y = trainingArray.reshape(-1,1)
    #X = np.array([-2, -1, 0, 1, 2, 1]).reshape(-1, 1)
    X = np.genfromtxt('dataset.csv', delimiter=',', dtype=np.float64).reshape(-1,1)
